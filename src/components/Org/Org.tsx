...
export const FormContent = ({setHide}: FormProps) => {
    ...
    return (<div className="form-door">
        ...
        <TextField variant="outlined" label="Nom de la porte" style={{minWidth: "80%", marginTop: "5vh" }} sx={{ input: { color: 'white' }}} onChange={(e) => {setName(e.target.value)}}/>
        <TextField variant="outlined" label="Hash de la porte" style={{minWidth: "80%", marginTop: "5vh" }} sx={{ input: { color: 'white'} }} onChange={(e) => {setHash(e.target.value)}}/>
        <Button style={{minWidth: "80%", backgroundColor: "#DCAE52", fontSize: "1.5em", color: "#292C35", marginTop: "10vh"}} onClick={() => {
            createPorte()
            setHide(false)
        }}>
            Ajouter
