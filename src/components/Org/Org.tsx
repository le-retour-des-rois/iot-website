...
export const Org = () => {
    ...
    useEffect(() => {
        if (!isloaded) {
            const res = axios.get(`http://34.89.98.217:8400/door/Le%20Retour%20Des%20Rois/${section}/`).then((rep) => {
                console.log(rep)
                setDoors(rep.data)
                setLoaded(true)
    ...
    const deleteDoor = (name: string) => {
        const res = axios.delete(`http://34.89.98.217:8400/door/Le%20Retour%20Des%20Rois/${section}/${name}`).then((rep) => {
...
export const FormContent = ({ setHide }: FormProps) => {
    ...
    const section = uri[uri.length - 1].replace("%20", " ")
    const createPorte = () => {
        axios.post('http://34.89.98.217:8400/door', { name: name, hash: hash, organization_name: "Le Retour Des Rois", section_name: section }).then((rep) => { console.log(rep) }).catch((err) => { console.log(err) })
    }
