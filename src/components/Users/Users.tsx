...
export const Users = () => {
    ...
    useEffect(() => {
        if (!isloaded) {
            const res = axios.get(`http://34.89.98.217:8400/user`).then((rep) => {
                const tmp: any[] = []
...
export const FormContent = ({ setHide, setLoaded }: FormProps) => {
    ...
    const createUser = () => {
        axios.post('http://34.89.98.217:8400/user', { username: name, type: rank.toLowerCase(), organization_name: "Le Retour Des Rois", mac_addr: mac, password: password }).then((rep) => { console.log(rep) }).catch((err) => { console.log(err) })
    }
